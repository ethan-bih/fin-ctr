      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs">
        <div>
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <span>Danh Sách 6 Mâm Quả &amp; Lễ Vật Đám Hỏi</span>
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-pink-50 text-pink-700 border border-pink-200">
              {preparedCount} / {gifts.length} mâm hoàn tất
            </span>
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            Checklist chuẩn bị mâm quả truyền thống (Trầu cau, Trà rượu, Bánh phu thê, Trái cây rồng phụng...)
          </p>
        </div>

        <button
          onClick={() => onOpenGiftModal()}
          className="flex items-center justify-center space-x-2 px-4 py-2.5 rounded-xl bg-pink-600 hover:bg-pink-700 text-white font-semibold text-xs sm:text-sm shadow-xs transition-all shrink-0"
        >
          <Plus className="w-4 h-4" />
          <span>Thêm Mâm Quả Mới</span>
        </button>
      </div>
